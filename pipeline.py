...
def parse_doc(
    ...
    try:
        ...
        if translate_to_english or generate_pdf or fix_md or translate_images or generate_config:
            ...
            try:
                from post_processing.post_process_runner import run_post_processing
...
if __name__ == '__main__':
    ...
    for doc_path in Path(pdf_files_dir).glob('*'):
        if guess_suffix_by_path(doc_path) in pdf_suffixes + image_suffixes:
            ...
            # 若输出目录已存在对应结果，则跳过，避免重复处理
            existing_dir = os.path.join(output_dir, file_name)
            if os.path.isdir(existing_dir):
                logger.info(f"检测到已存在结果目录，跳过: {file_name}")
                continue
